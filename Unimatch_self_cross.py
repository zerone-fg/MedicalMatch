...
def main():
    ...
    trainloader_l = DataLoader(trainset_l, batch_size=cfg['batch_size'], shuffle=True,
                               num_workers=0, drop_last=True)
    trainloader_u = DataLoader(trainset_u, batch_size=cfg['batch_size'], shuffle=True,
                               num_workers=0, drop_last=True)
    trainloader_u_mix = DataLoader(trainset_u, batch_size=cfg['batch_size'], shuffle=True,
                                   num_workers=0, drop_last=True)
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        total_cos_l = AverageMeter()
        total_cos_u = AverageMeter()
        ...
        for i, ((img_x, mask_x),
            ...
            pred_u_w, pred_u_w_fp = model(img_u_w, need_fp=True)
            
            pred_u_s1 = model(img_u_s1, cluster=True)
            pred_u_s3 = model(img_u_s1)
            
            pred_x, pred_u_s2 = model(torch.cat((img_x, img_u_s2), dim=0), mask_x, cluster=True, cls_aug=True)
            ...
            loss_u_s3 = criterion_dice(pred_u_s3.softmax(dim=1), mask_u_w_cutmixed1.unsqueeze(1).float(),
                                        ignore=(conf_u_w_cutmixed1 < cfg['conf_thresh']).float())
            
            loss_u_w_fp = criterion_dice(pred_u_w_fp.softmax(dim=1), mask_u_w.unsqueeze(1).float(),
                                          ignore=(conf_u_w < cfg['conf_thresh']).float())

            loss = (loss_x + loss_u_s1 * 0.25 + loss_u_s2 * 0.25 + loss_u_w_fp * 0.5 + loss_u_s3 * 0.25) / 2.0
            ...
            total_loss_s.update((loss_u_s1.item() + loss_u_s2.item() + loss_u_s3.item()) / 3.0)
            ...
            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                logger.info(
                    ...
                    '{:.3f}, loss_cos_l: {:.3f}, loss_cos_w: {:.3f}'.format(i, total_loss.avg, total_loss_x.avg,
                                                                            total_loss_s.avg,
                                                                            total_loss_w_fp.avg, total_mask_ratio.avg,
                                                                            total_cos_l.avg, total_cos_u.avg))
