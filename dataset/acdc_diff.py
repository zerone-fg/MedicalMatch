...
class ACDCDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        img_s1, img_s2, img_s3 = deepcopy(img), deepcopy(img), deepcopy(img)
        ...
        if random.random() < 0.8:
            img_s3 = transforms.ColorJitter(0.5, 0.5, 0.5, 0.25)(img_s3)
        img_s3 = blur(img_s3, p=0.5)
        img_s3 = torch.from_numpy(np.array(img_s3)).unsqueeze(0).float() / 255.0

        # ignore_mask = Image.fromarray(np.zeros((mask.size[1], mask.size[0])))

        return img, img_s1, img_s2, cutmix_box1, cutmix_box2, img_s3
